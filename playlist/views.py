...
@permission_required('playlist.start_stream')
def start_jingleplayer(request):
  utils.start_jingleplayer()
  return HttpResponseRedirect(reverse('g2admin'))
...
def ajax(request):
  ...
  if server_playing.id != client_playing:
    ...
    comments = server_playing.song.comments.all().order_by(
      "time")  # ensure oldest first - new comments are placed at top of update list
...
def listartists(request, letter='123', page='1'):
  ...
  if letter == '123':
    ...
    artists = Artist.objects.filter(sort_name__istartswith=letter).order_by("sort_name").annotate(
...
def user(request, userid):
  ...
  curotheradds = PlaylistEntry.objects.select_related().filter(song__uploader__id=owner.id).exclude(
    adder__id=owner.id).count()
  otheradds = OldPlaylistEntry.objects.select_related().filter(song__uploader__id=owner.id).exclude(
    adder__id=owner.id).count() + curotheradds
...
def upload(request):
  if request.method == "POST":
    ...
    if form.is_valid():
      ...
      for f in files:
        try:
          request.user.userprofile.uploadSong(
            UploadedFile(f.temporary_file_path(), f.name, username=request.user.username))
        except DuplicateError:
          messages.add_message(request, messages.ERROR, f.name + ": error: track already uploaded")
        except FileTooBigError:
          messages.add_message(request, messages.ERROR, f.name + ": error: file too big")
        except StorageError:
          messages.add_message(request, messages.ERROR, f.name + ": error: file could not be stored!")
        else:
          messages.add_message(request, messages.ERROR, f.name + ": uploaded file successfully!")
  else:
    form = UploadFileForm()
...
def globalstats(request):
  ...
  try:
    with open('/tmp/top10adders_30days.json') as f:
  ...
  try:
    with open('/tmp/top10uploaders_alltime.json') as f:
      popularuploaders = ast.literal_eval(f.read())
  except:
    pass

  try:
    with open('/tmp/top10uploaders_30days.json') as f:
      popularuploaders30 = ast.literal_eval(f.read())
  except:
    pass
...
def artist(request, artistid=None):
  try:
    ...
    raise Http404
  print(artist)
  songs = Song.objects.select_related("artist", "album").check_playable(request.user).filter(artist=artist).order_by(
    "album__name", "track")
  print(songs)
...
def add(request, songid=None):
  ...
  if oldtokens != profile.tokens:
    if profile.tokens:
      msg = "You already had a dong on the playlist, so you've used up a token to add this one. You have %d left" % (
        profile.tokens)
    else:
  ...
  if request.method == "POST":
    ...
    if form.is_valid():
      ...
      if error is None:
        ...
        try:
          g = Group.objects.get(name="Listener")
